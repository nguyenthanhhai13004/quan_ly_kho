...
class AdvisorRequestRepository {
  async getAll(params: PaginationRequestsDto): Promise<ResponsePaginationDto<any>> {
    const { size = 20, page = 1, keyword = "", status, type, class_id } = params;
  ...
  async getByAdvisor(advisorId: number, params: PaginationRequestsDto): Promise<ResponsePaginationDto<any>> {
    // Filter directly in MySQL by the advisor's own id.
    const { size = 20, page = 1, keyword = "", status, type } = params;
    const offset = (page - 1) * size;
