...
export function sqlInsertToJson(sql: string): Row[] {
  const results: Row[] = [];

  // 1️⃣ Match NHIỀU INSERT blocks
  const insertRegex =
  ...
  while ((insertMatch = insertRegex.exec(sql)) !== null) {
    ...
    // 3️⃣ VALUES block (có thể nhiều dòng)
    const valuesBlock = insertMatch[2];

    // 4️⃣ Match từng record: ( ... )
    const rowRegex = /\(([^()]+)\)/g;
    ...
    while ((rowMatch = rowRegex.exec(valuesBlock)) !== null) {
      ...
      // 5️⃣ Split values (an toàn với dấu phẩy trong string)
      const values = rawValues.match(/'(?:''|[^'])*'|[^,]+/g);
...
const json = sqlInsertToJson(sql);
